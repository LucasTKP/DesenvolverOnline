...
function Hero() {
    return (
        <section id={'home'} className='bg-[#F1F1F1] px-[10%] max-2xl:px-[9%] max-xl:px-[8%] max-lg:px-[6%] max-md:px-[4%] max-sm:px-[3%] flex max-sm:flex-col items-center justify-between relative overflow-x-hidden'>
            ...
            <a href='#services' className='flex items-center p-[8px] max-lg:p-[6px] max-md:p-[4px] bg-primary rounded-full aspect-square absolute left-1/2 -translate-x-1/2 bottom-[20px] animate-bounce hover:brightness-75 duration-200 max-sm:hidden'>
                <Image src={arrowBottom} alt='Flecha' width={20}  className='max-lg:w-[18px] max-md:w-[16px]'/>
            </a>


            <div className='max-sm:mt-[20px]'>
                <h1 className='text-[50px] max-2xl:text-[40px] max-xl:text-[34px] max-lg:text-[26px] max-md:text-[23px] max-sm:text-[26px] max-lsm:text-[20px] font-medium max-sm:text-center'>Agencia Digital <br className='max-sm:hidden' /> <span className='text-primary font-semibold'>Especializada</span> no <br /> desenvolvimento do <br className='max-sm:hidden' /> seu <span className='text-primary font-semibold'>negócio</span>.</h1>
